...
PositionTracker.prototype.optimize = function(iter_max, ray1, ray2, ray3, collapsed) {
	var min_dir = ray1.clone().cross(ray2);
...
*/
		...
		ray.direction = ray3.clone();	// ray1<->ray3 is diagonal
		crossbar_plane.normal = crossbar_dir.clone();
		crossbar_plane.constant = -crossbar_dir.dot(center);
		var world_pos3 = ray.intersectPlane(crossbar_plane);
		if (world_pos3) {
			var world_pos;
			if (collapsed) {
				...
				world_pos = world_pos1.clone().add(offset.multiplyScalar(div));
			} else {
				world_pos = world_pos1;
			}
//			var world_pos = collapsed ? world_pos1.clone().add(world_pos2).multiplyScalar(0.5) : world_pos1;
			cast_center.addVectors(world_pos, world_pos3);	// center of the diagonal
...
function match_dots (old_dots, new_dots) {
	var few_dots, more_dots;
	var old_is_few = old_dots.length <= new_dots.length;
	if (old_is_few) {
		few_dots = old_dots;
		more_dots = new_dots;
	} else {
	...
	var dist = [];
	for (var i=0; i<few_dots.length; i++) {
		dist[i] = [];
		var di = few_dots[i];
		for (var j=0; j<more_dots.length; j++) {
			var dj = more_dots[j];
			dist[i][j] = di.screen_pos.distanceToSquared(dj.screen_pos);
		}
	}

	var few = few_dots.length;
	var more = more_dots.length;
	var min_total_dist = 1e10;
	var min_tuple;
	var j = [];

	for (j[0]=0; j[0]<more; j[0]++) {

		for (j[1]=0; j[1]<more; j[1]++) {
			if (j[1] === j[0]) continue;

			for (j[2]=0; j[2]<more; j[2]++) {
				if (j[2] === j[0] || j[2] === j[1]) continue;

				for (j[3]=0; j[3]<more; j[3]++) {
					if (j[3] === j[0] || j[3] === j[1] || j[3] === j[2]) continue;

					var total_dist = 0;
					var tuple = [];
					for (var i=0; i<few; i++) {
						total_dist += dist[i][j[i]];
						tuple[i] = j[i];
					}
...
function collapse4to3 (old_4dots, new_3dots) {
	// edge or diagonal?
	// diagonal collapse is irrelevent

	// find closest dots between old and new
	var min_dist = 1e10;
	var min_i, min_j;
	for (var i=0; i<4; i++) {
		var od = old_4dots[i];
		for (var j=0; j<3; j++) {
			var nd = new_3dots[j];
			var dist = od.screen_pos.distanceToSquared(nd.screen_pos);
			if (dist < min_dist) {
				min_i = i;
				min_j = j;
			}
	...
	// shift by min_dist for better matching
	var new_screen_pos = [];
	var shift = new_3dots[min_j].screen_pos.clone().sub(old_4dots[min_i].screen_pos);
	for (var j=0; j<3; j++) {
		new_screen_pos[j] = new_3dots[j].screen_pos.clone().sub(shift);
	}
...
//			iter_max = 1;
	...
	if (ir_dots.length === 4 && new_cnt === 2 && this.record && this.record.frame === this.frame_cnt) {
		...
		for (k=0; k<2; k++) {
			...
			var result2 = this.optimize(iter_max, r1, r2, r3);
...
//			this.debug_dot(result.world_pos2, 0x00dd00, 5);
			...
			// 2 kinds of collapse: edge & diagonal
			...
			*/
			...
			if (old_cnt === 4) {
				...
				// real_dots idx -> ir_dots idx
				collapsed_i = min_dot_tuple[2];
				for (var i=0; i<4; i++) {
					if (ir_dots[i] == real_dots[collapsed_i]) {
						min_dot_tuple[2] = i;
						break;
					}
				}

				var min_dot = ir_dots[min_dot_tuple[2]];
...
//			this.collapsed = false;
			...
			if (this.collapsed) {
				min_i = min_dot_tuple[2];
				d1 = real_dots[(collapsed_i+1)%3];
				d2 = real_dots[(collapsed_i+2)%3];
				d3 = real_dots[collapsed_i];
//				console.log(collapsed_i, real_dots);

				ray1 = d1.world_pos.clone().sub(camera_saved.position).normalize(); //ir_rays[min_i].clone();
				ray2 = d2.world_pos.clone().sub(camera_saved.position).normalize();//ir_rays[(min_i+1)%3].clone();
				ray3 = d3.world_pos.clone().sub(camera_saved.position).normalize();//ir_rays[(min_i+2)%3].clone();
			} else {
				for (var i=0; i<4; i++) {
					var j = (i+1)%4;
					var d1 = ir_dots[i];
...
//			var diagonal_dot_dir;
			...
			var best_test_euler = test_euler.clone();
			result = this.optimize(iter_max, ray1, ray2, ray3, this.collapsed);//diagonal_dot_dir, d3.guess);
			var center_screen_pos = world_to_screen(result.center);
			...
			if (this.collapsed) {
				...
				var edge_center1 = real_dots[i].screen_pos.clone().add(real_dots[collapsed_i].screen_pos).multiplyScalar(0.5);
				var edge_center2 = real_dots[j].screen_pos.clone().add(real_dots[collapsed_i].screen_pos).multiplyScalar(0.5);
				var offset = edge_center1.clone().sub(edge_center2);
//				diag_center_screen_pos = edge_center2.add(offset.multiplyScalar(1.0/3));
				diag_center_screen_pos = real_dots[i].screen_pos.clone().add(real_dots[j].screen_pos).multiplyScalar(0.5).add(real_dots[collapsed_i].screen_pos).multiplyScalar(0.5);
			} else {
				var diag_d = 0;
				for (i=0; i<2; i++) {
					for (var j=i+1; j<3; j++) {
...
/*			if (dist1 > 10) {
				test_euler.set(0,0,0);
				var result2 = this.optimize(iter_max, ray1, ray2, ray4);
				var center2_screen_pos = world_to_screen(result2.center);
...
//			console.log('dist1',dist1, [min_i, (min_i+1)%4, (min_i+2)%4]);
			var min_tuple;
			if (dist1 > 10) {
//				console.log('bad 3');
				for (var i=0; i<4; i++) {
...
//			console.log('dist2',dist1,min_tuple);
			test_euler.copy(best_test_euler);
...
//			test_euler.set(0,0,0);
			result = this.optimize(iter_max, ray1, ray2, ray3);//diagonal_dot_dir, d3.guess);
